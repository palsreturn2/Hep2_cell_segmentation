...
void genFeatures(GrayImage *img,GrayImage *fgm,GrayImage *simg)
{
	...
	temp=imlabel(fgm,&labels,&nl);
	FILE *fp=fopen("featuresLBP.txt","w");
	...
	for(k=0;k<nl;k++)
	{
		...
		//Enter your function
		if(cropped.row>1 && cropped.col>1)
		{	
			//printf("calculating LBP for %d\n%d %d\n",k,cropped.row,cropped.col);
			//imshow(cropped);
...
main(int argc,char **argv)
{
	...
	readPGM(argv[2],&fgm);
	readPGM(argv[3],&bgm);
	bwareaopen(&fgm,&nl,100);
	fgm=imerode(&fgm,diskstrel(8),17);
	//fgm.row=300;
	//fgm.col=300;
	//genBgm(&img,&nl,&bgm);
